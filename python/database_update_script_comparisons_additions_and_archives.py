...
layout = [  [gui.Text('IMPORTING, CONVERTING, AND COMPARING FILES', font=(standard_font))],
...
files_used = ['occupationdataadditions.csv',
    'contentmodelreferenceadditions.csv',
    'content_model_relationships.csv',
    'SOCMajorGroup.csv',
    'SOC_Level_With_Detailed.csv',
    'SOC_Level_Without_Detailed.csv',
    'DetailedOccupation.csv',
    'scalesreferenceadditions.csv',
    'abilitiesadditions.csv',
    'alternatetitlesadditions.csv',
    'iwareferenceadditions.csv',
    'dwareferenceadditions.csv',
    'educationtrainingandexperienceadditions.csv',
    'interestsadditions.csv',
    'jobzonereferenceadditions.csv',
    'jobzonesadditions.csv',
    'knowledgeadditions.csv',
    'skillsadditions.csv',
    'taskstatementsadditions.csv',
    'taskratingsadditions.csv',
    'taskstodwasadditions.csv',
    'unspscreferenceadditions.csv',
    'technologyskillsadditions.csv',
    'toolsusedadditions.csv',
    'workactivitiesadditions.csv',
    'workstylesadditions.csv',
    'ncc_crosswalk.csv']
#TODO: add files used in employee data to this list

log_path = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'logs'))
comparison_counter = 1 # number of comparisons, manually increment through loop
comparison_times = [0] # list to track execution time of comparisons (used for logging)
comparison_times_and_summary_log_file = open(os.path.join(log_path, 'comparison_times_and_summary_logs.txt'), 'w+') # create query exection time log file
...
for link in soup.find_all('a'):
    ...
    if 'href' in link.attrs:
        if '.xlsx' in link.attrs['href']:
            ...
            if xlsx_file_name.replace('.xlsx', 'additions.csv') in files_used:
                ...
                if firstrun:
                    csv_file_name = xlsx_file_name.replace('.xlsx', 'additions.csv') # if fresh installing, treat whole file as 'additions'
                    ...
                    import_count += 1
                if updaterun:
                    # if you're updating the database, check that the archived_file exists before comparing it to the csv_file
                    csv_file_name = xlsx_file_name.replace('.xlsx', '.csv')
                    archive_exists = False
                    archive_file_name = csv_file_name.replace('.csv', 'archive.csv')
                    for existing_file in os.listdir(path):
                        if existing_file == archive_file_name:
                            archive_exists = True
                    # if there's no archived file, just create the new additions_file like you do on a fresh install and use that instead
                    if not archive_exists:
                        csv_file_name = xlsx_file_name.replace('.xlsx', 'additions.csv')
                        csv_file = open(os.path.join(path, csv_file_name), 'w')
                        wr = csv.writer(csv_file, quoting=csv.QUOTE_ALL)
                        for rownum in range(sheet.nrows):
                            wr.writerow(sheet.row_values(rownum))
                        ...
                        os.remove(os.path.join(path, xlsx_file_name))
                        #logging
                        print(archive_file_name + ' does not exist, creating ' + csv_file_name)
                        print('Total Imported: ' + str(import_count))
                        print('Total Skipped: ' + str(skip_count))
                        window.read(timeout=0.1) #timeout was the make or break piece
                        window['recent_file'].update(archive_file_name + ' does not exist, creating ' + csv_file_name)
                        window['import'].update('Total Imported: ' + str(import_count))
                        window['skip'].update('Total Skipped: ' + str(skip_count))
                        import_count += 1
                    # if there is an archived file, find diff between it and csv_file -> produce new additions file
                    else: 
                        # convert xlsx file to csv_file
                        csv_file = open(os.path.join(path, csv_file_name), 'w')
                        wr = csv.writer(csv_file, quoting=csv.QUOTE_ALL)
                        for rownum in range(sheet.nrows):
                            wr.writerow(sheet.row_values(rownum))
                        ...
                        # remove unnecessary xlsx
                        os.remove(os.path.join(path, xlsx_file_name))
                        #diff archive and csv_file
                        additions_file_name = csv_file_name.replace('.csv', 'additions.csv')
                        archive_file = open(os.path.join(path, archive_file_name), 'r')
                        archive_file_contents = archive_file.readlines()
                        csv_file = open(os.path.join(path, csv_file_name), 'r')
                        csv_file_contents = csv_file.readlines()
                        additions_file = open(os.path.join(path, additions_file_name), 'w')
                        # write differences to the new additions file
                        diff_time_start = time.perf_counter() # start timer to log time it takes to get file diffs
                        for line in csv_file_contents:
                            # always write the first line (header)
                            if line[0]:
                                additions_file.write(line)
                            if line not in archive_file_contents:
                                additions_file.write(line)
                        diff_time_stop = time.perf_counter() # stop timer to log time it takes to get file diffs
                        comparison_times.append(diff_time_stop - diff_time_start) # add comparison execution time to the list
                        additions_file.close()
                        csv_file.close()
                        archive_file.close()
                        # remove old archive file, rename new csv_file to archive_file_name
                        os.remove(os.path.join(path, archive_file_name))
                        os.rename(os.path.join(path, csv_file_name), os.path.join(path, archive_file_name))
                        #logging
                        print('Compared '+archive_file_name+' and '+csv_file_name+ f' in {comparison_times[comparison_counter]:0.4f} seconds.')
                        print('Total Imported: ' + str(import_count))
                        print('Total Skipped: ' + str(skip_count))
                        window.read(timeout=0.1) #timeout was the make or break piece
                        window['recent_file'].update('Compared '+archive_file_name+' and '+csv_file_name+ f' in {comparison_times[comparison_counter]:0.4f} seconds.')
                        window['import'].update('Total Imported: ' + str(import_count))
                        window['skip'].update('Total Skipped: ' + str(skip_count))
                        comparison_times_and_summary_log_file.write('Compared '+archive_file_name+' and '+csv_file_name+ f' in {comparison_times[comparison_counter]:0.4f} seconds.\n')
...
comparison_times_and_summary_log_file.close()
time.sleep(5)
window.close()
...
FROM 'file:///occupationdataadditions.csv' AS line
RETURN line
","
WITH line
Limit 1
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///contentmodelreferenceadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
From 'file:///scalesreferenceadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///abilitiesadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///alternatetitlesadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///iwareferenceadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///dwareferenceadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///educationtrainingandexperienceadditions.csv' AS line
RETURN line
","
MATCH (a:Occupation {onet_soc_code: line.`O*NET-SOC Code`})
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///interestsadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///jobzonereferenceadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///jobzonesadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///knowledgeadditions.csv' AS line
RETURN line
","
MATCH (o:Occupation {onet_soc_code: line.`O*NET-SOC Code`})
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///skillsadditions.csv' AS line
RETURN line
","
MATCH (o:Occupation {onet_soc_code: line.`O*NET-SOC Code`})
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///taskstatementsadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///taskratingsadditions.csv' AS line
RETURN line
","
MATCH (o:Occupation {onet_soc_code: line.`O*NET-SOC Code`})
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///taskstodwasadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///unspscreferenceadditions.csv' AS line
RETURN line
","
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///technologyskillsadditions.csv' AS line
RETURN line
","
MATCH (o:Occupation {onet_soc_code: line.`O*NET-SOC Code`})
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///toolsusedadditions.csv' AS line
RETURN line
","
MATCH (o:Occupation {onet_soc_code: line.`O*NET-SOC Code`})
MATCH (m:Commodity {commodityID: toInteger(line.`Commodity Code`)})
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///workactivitiesadditions.csv' AS line
RETURN line
","
MATCH (o:Occupation {onet_soc_code: line.`O*NET-SOC Code`})
MATCH (a:Generalized_Work_Activities { elementID: line.`Element ID`})
...
query_list.append("""CALL apoc.periodic.iterate("
LOAD CSV WITH HEADERS
FROM 'file:///workstylesadditions.csv' AS line
RETURN line
","
MATCH (o:Occupation {onet_soc_code: line.`O*NET-SOC Code`})
...
# rename additions files on the first run to be the first set of archived files
if firstrun or (updaterun and not archive_exists):
    for files in os.listdir(path):
        new_file_name = files.replace('additions','archive')
        os.rename(os.path.join(path, files), os.path.join(path, new_file_name))
